...
public class SelectionB implements GenerticAlgorithmB {
	...
	public SelectionB(Population population) {
...
//		ArrayList<Individual> newList = (ArrayList<Individual>) Arrays.asList(listOfPopulation);
		...
		for (int i = 0; i < 10000; i++) {
			int tempComparer = rand.nextInt(100);
			if (!newList.contains(tempComparer))
				newList.add(tempComparer);
		}
		...
		if (selectionReturnSize % 2 == 1)
			selectionReturnSize = -1;
		...
		for (int i = 0; i < selectionReturnSize / 2; i++) {
			...
			if (indAToBeReturned == indBToBeReturned)
				do
